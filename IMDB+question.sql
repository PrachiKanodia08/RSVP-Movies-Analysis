...
SELECT
	count(id) AS number_of_movies_USA_India
FROM movie
WHERE
...
SELECT
	DISTINCT(genre)
FROM genre;
-- There are 13 genres of movies in the dataset.
...
-- Joining tables movies and genre to get the number of movies released in each genre
-- Getting the genre with highest number of movies using LIMIT clause
SELECT
    genre,
...
HAVING
	count(genre) = 1
)SELECT count(*) AS 'Movies with one genre' FROM movie_genre_count_one;
...
-- Grouping the movies accoding to the genre and finding average duration in each group  
SELECT     genre,
...
GROUP BY
	genre;
-- Thriller genre has rank 3 with total movie count of 1484
...
-- Getting  MIN and MAX values of each column
SELECT 
	   MIN(avg_rating)    AS min_avg_rating,
       MAX(avg_rating)    AS max_total_votes,
...
ON r.movie_id = m.id
LIMIT 10;
-- 3 movies have average rating >= 9.8
...
ORDER BY 
	median_rating;
-- 2257 movies have median rating of 7 and is the highest
...
-- Dream Warrior Pictures and National Theatre Live production houses have produced the most number(3) of hit movies (average rating > 8)
...
-- 24 Drama movies were released during March 2017 in USA having more than 1000 votes
-- Top 3 genres are drama, comedy and action during March 2017 in the USA and had more than 1,000 votes
-- Movies from 12 genres had more than 1,000 votes
...
-- 361 movies were released in between 1 April 2018 and 1 April 2019 with median rating of 8
...
-- By observation, German movies recieved 4695 votes where as Italian movies recieved 1684 votes.
-- German movies received the more number of votes compared to Italian movies.
...
-- Get the top 3 genres using average rating > 8 condition and highest movie counts
-- Using the top genres derived, the directors are derived whose movies have an average rating > 8 and are sorted based on number of movies. 
...
-- James Mangold , Joe Russo and Anthony Russo are top three directors in the top three genres whose movies have an average rating > 8 with movie count of 4,3,3 respectively
...
ORDER  BY movie_count DESC
LIMIT  2; 
-- Mammootty and Mohanlal are the top 2 actors having their movie median rating >= 8.
...
GROUP BY production_company
LIMIT 3;
/* Top three production houses based on the number of votes received by their movies are Marvel Studios(votes: 2656967)
, Twentieth Century Fox(votes: 2411163) and Warner Bros(votes: 2396057 ).*/
...
-- CASE statements to classify thriller movies as per avg rating 
...
LIMIT 2; 
/* Star Cinema and Twentieth Century Fox are the top two production houses with movie count 7 and 4 respectively 
